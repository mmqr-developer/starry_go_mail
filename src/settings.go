...
var settingDefs = []Setting{
	...
	{Key: "reading.show_envelope", Scope: ScopeMailbox, Section: "general",
		Label: "Show the envelope addresses",
		Help:  "Adds Envelope From (Return-Path) and Envelope To (Delivered-To) under the To line. These are what the mail SERVERS used to route the message, and they can differ from the From and To the sender wrote -- which is what makes them worth seeing when a message looks like it came from somewhere it did not.",
		Kind:  "bool", Default: "0"},
